...
# USER CONFIG
# --------------------
KILNS_CSV = "/home/rishabh.mondal/Brick-Kilns-project/ijcai_2025_kilns/after_hand_validation_data/gdf_kilns_new.csv"
...
region = "all_kilns"
out_dir = f"data_all/{region}_y_{year}_z_{zoom}_buf_{buffer_m}m"
...
# --------------------
# Load kilns CSV -> GeoDataFrame (BOX geometry)
# --------------------
df = pd.read_csv(KILNS_CSV)

required = {"min_lon", "min_lat", "max_lon", "max_lat"}
missing = required - set(df.columns)
if missing:
    raise ValueError(f"KILNS_CSV missing columns: {missing}. Need min_lon/min_lat/max_lon/max_lat.")

geometry = [box(r.min_lon, r.min_lat, r.max_lon, r.max_lat) for r in df.itertuples(index=False)]
kilns_gdf = gpd.GeoDataFrame(df, geometry=geometry, crs="EPSG:4326").to_crs("EPSG:4326")

# centroid columns
kilns_gdf["center_lon"] = kilns_gdf.geometry.centroid.x
kilns_gdf["center_lat"] = kilns_gdf.geometry.centroid.y

# Optionally drop invalid centroids
kilns_gdf = kilns_gdf.dropna(subset=["center_lat", "center_lon"]).reset_index(drop=True)

print("Total kilns loaded:", len(kilns_gdf))

# --------------------
# Download + crop for ALL kilns
# --------------------
results = []
